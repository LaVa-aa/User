...
struct mapView: View {
    ...
    @State private var region = MKCoordinateRegion(center: CLLocationCoordinate2D(latitude: 57, longitude: 14), span: MKCoordinateSpan(latitudeDelta: 1, longitudeDelta: 1))

    //Make a temporary cllCoordinate - it is edited when i get the users
    @State private var cllCoordinate = CLLocationCoordinate2D(latitude: 57, longitude: 14)
